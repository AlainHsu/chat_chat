...
import 'package:chat_chat/Models/chatContentModel.dart';
...
class ChatListProvider with ChangeNotifier {
  ...
  ChatListProvider() {
    ...
    data.forEach((element) {
      users.add(UserModel(
          ...
          isMale: element['gender'] > 0 ? true : false,
    ...
    users.forEach((element) {
      ...
      chats.add(ChatContentModel(
          isRead: false,
          lastContent: 'This is the last message.',
  ...
  String getRandomTime() {
    ...
    DateTime rdTime =
        DateTime.fromMillisecondsSinceEpoch(now.millisecondsSinceEpoch - rd);

    Duration diff = now.difference(rdTime);
    if (diff.inDays > 1) {
